...
|---|---|---|
| "Tests pass" | Run test command: 0 failures, 0 errors | Previous run, "should pass", looks right |
| "Bug fixed" | Reproduce original symptom; verify it no longer happens | Code changed, assumed fixed |
| "Agent completed task" | VCS diff shows the actual changes | Agent reports "done" / "success" |
| "Regression test works" | Red → Green → Revert → Restore: full cycle | Test passes once on current code |
| "No regressions" | Run full test suite after change | Run only the changed area's tests |
| "Lint clean" | Run lint with 0 errors, 0 warnings | "lint should pass after my changes" |

The "trust agent report" row is the most common failure mode in multi-agent workflows.
The regression red-green-revert-restore cycle proves the test actually catches the bug
(if you can revert the fix and watch the test go red, the test is real).
